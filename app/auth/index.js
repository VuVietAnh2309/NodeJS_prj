...
function setup(app, session, core) {
    ...
    async function tokenAuth(username, password, done) {
        if (!done) {
            done = password;
        }
        var User = mongoose.model("User");
        user = await User.findByToken(username);
        if (!user) {
...
function checkIfAccountLocked(username, cb) {
    ...
    var isLocked =
        attempt && attempt.lockedUntil && attempt.lockedUntil > Date.now();
                ...
                var lock = Math.min(
                    5000 *
                        Math.pow(
                            2,
                            attempt.attempts -
                                settings.auth.throttling.threshold,
                            MAX_AUTH_DELAY_TIME
                        )
                );
...
function authenticate() {
    var req, username, cb;
    // console.log("authenticate.1",req,username,cb);
    ...
    checkIfAccountLocked(username, function (locked) {
        ...
        if (settings.auth.throttling && settings.auth.throttling.enable) {
        ...
        var series = enabledProviders.map(function (p) {
            ...
            return function () {
                ...
                provider.authenticate(req, function (err, user) {
                    // console.log("provider.authenticate", user, err);
                    if (err) {
