...
(function (window, $, _) {
            ...
            function (resRoom) {
                ...
                that.getMessages(
                    ...
                    function (messages) {
                        ...
                        that.addMessages(
                            messages,
                            !rejoin && !room.lastMessage.get("id")
                        );
                        !rejoin &&
    ...
    Client.prototype.calcAlert = function (roomId, userId, callback) {
        const options = {
            roomId: roomId,
            userId: userId,
        };
        this.socket.emit("rooms:calcAlert", options, callback);
    };
    Client.prototype.clearAlerts = function (roomId, userId, callback) {
        const options = {
            roomId: roomId,
            userId: userId,
        };
        this.socket.emit("rooms:clearAlerts", options, function () {});
    };
    //
    // Messages
    //
    ...
    Client.prototype.listen = function () {
        ...
        this.socket.on("rooms:addAdmin", function (user) {
            if (user.id === that.user.id) {
                that.getRooms(function (rooms) {
                    var roomIds = _.map(rooms, function (room) {
                        return room.id;
                    });
                    var openRooms = user.openRooms || [];
