...
+(function (window, $, _) {
    ...
    window.LCB.TabsView = Backbone.View.extend({
        ...
        add: async function (room) {
            let total = 0;
            total = await new Promise((resolve) => {
                this.client.calcAlert(
                    room.id,
                    this.client.user.id,
                    function (alertCount) {
                        resolve(alertCount);
        ...
        update: function (room) {
            this.$el
                .find(".lcb-tab[data-id=" + room.id + "] .lcb-tab-title")
                .text(room.get("name"));
        },
        switch: function (id) {
            if (!id) {
        ...
        alert: async function (message) {
            ...
            var mentions = parseInt($tab.data("count-mentions")) || 0;
            $tab.data("count-total", totalMessage);
            $total.text(totalMessage);
            // Just mentions
            if (
                new RegExp(
                    "\\B@(" + this.client.user.get("username") + ")(?!@)\\b",
                    "i"
                ).test(message.text)
            ) {
        ...
        clearAlerts: function (id) {
            ...
            $mentions.text("");
            if (id !== "list") {
                this.client.clearAlerts(id, this.client.user.id);
            }
        },
