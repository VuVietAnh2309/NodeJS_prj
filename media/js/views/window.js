...
+(function (window, $, _, notify) {
    ...
    window.LCB.WindowView = Backbone.View.extend({
        ...
        onNewMessage: function (message) {
            if (
                this.focus ||
                message.historical ||
                message.owner.id === this.client.user.id
            ) {
                return;
            }
        ...
        flashFaviconBadge: function () {
            if (!this.faviconBadgeTimer) {
                ...
                this.faviconBadgeTimer = setInterval(
                    flashFaviconBadge,
                    1 * 2000
                );
            }
    ...
    window.LCB.HotKeysView = Backbone.View.extend({
        ...
        recallRoom: function () {
            this.client.events.trigger(
                "rooms:switch",
                this.rooms.last.get("id")
            );
        },
        toggleRoomSidebar: function (e) {
            e.preventDefault();
            var view =
                this.client.view.panes.views[this.rooms.current.get("id")];
    ...
    window.LCB.DesktopNotificationsView = Backbone.View.extend({
        ...
        onFocusBlur: function (e) {
            this.focus = e.type === "focus";
            _.each(
                _.merge(this.openNotifications, this.openMentions),
