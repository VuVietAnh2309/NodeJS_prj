...
+(function (window, $, _) {
    ...
    window.LCB.RoomView = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            this.messageTemplate = Handlebars.compile(
        ...
        goodbye: function () {
            swal(
                "Archived!",
                '"' + this.model.get("name") + '" has been archived.',
                "warning"
            );
        },
        updateMeta: function () {
            ...
            this.$(".lcb-room-heading .slug").text(
                "#" + this.model.get("slug")
            );
            this.$(".lcb-room-participants").text(
                this.model.get("participants")
            );
            this.formatMessage(
                _.escape(this.model.get("description")),
                function (html) {
        ...
        submitEditRoom: function (e) {
            ...
            var $modal = this.$(".lcb-room-edit"),
                ...
                $confirmPassword = $modal.find('input[name="confirmPassword"]'),
                $participants = this.$(
                    '.edit-room textarea[name="participants"]'
                );
        ...
        archiveRoom: function (cb) {
            var that = this;
            swal(
                {
                    title: "Bạn có muốn kết thúc cuộc trò chuyện ? ",
                    type: "error",
        ...
        addMessage: function (message) {
            ...
            // WHATS MY NAME
            message.mentioned = new RegExp(
                "\\B@(" + this.client.user.get("username") + "|all)(?!@)\\b",
                "i"
            ).test(message.text);
        ...
        toggleSidebar: function (e) {
            ...
            this.$el
                .siblings(".lcb-room")
                .andSelf()
                .toggleClass("lcb-room-sidebar-opened");
        ...
        updateUser: function (user) {
            var $messages = this.$(
                '.lcb-message[data-owner="' + user.id + '"]'
            );
            $messages
                .find(".lcb-message-username")
                .text("@" + user.get("username"));
            $messages
                .find(".lcb-message-displayname")
                .text(user.get("displayName"));
        },
        endChat: function () {
            this.archiveRoom(function () {
                const originUrl = localStorage.getItem("originUrl");
                if (originUrl) {
                    localStorage.clear("originUrl");
    ...
    window.LCB.RoomSidebarListView = Backbone.View.extend({
        ...
        count: function (models) {
            this.$(".lcb-room-sidebar-items-count").text(
                this.collection.length
            );
        },
        update: function (model) {
            this.$(
                ".lcb-room-sidebar-item[data-id=" + model.id + "]"
