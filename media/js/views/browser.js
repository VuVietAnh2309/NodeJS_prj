    ...
    window.LCB.BrowserView = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            this.template = Handlebars.compile(
                $("#template-room-browser-item").html()
            );
            this.userTemplate = Handlebars.compile(
                $("#template-room-browser-item-user").html()
            );
            ...
            this.rooms.on(
                "change:lastActive",
                _.debounce(this.updateLastActive, 200),
                this
            );
        ...
        updateToggles: function (room, joined) {
            this.$(".lcb-rooms-switch[data-id=" + room.id + "]").prop(
                "checked",
                joined
            );
        },
        toggle: function (e) {
            ...
            var $target = $(e.currentTarget),
                $input =
                    ($target.is(":checkbox") && $target) ||
                    $target.siblings('[type="checkbox"]'),
                id = $input.data("id"),
        ...
        add: function (room) {
            ...
            //test
            // this.client.getUser();
            // this.client.getRooms(function (rooms, client) {
            //     var roomIds = _.map(rooms, function (room) {
            //         return room.id;
            //     });
            //     var openRooms = client.user.get("openRooms") || [];
            //     console.log("🚀 ~ openRooms fake:", openRooms);
        ...
        update: function (room) {
            this.$(
                ".lcb-rooms-list-item[data-id=" +
                    room.id +
                    "] .lcb-rooms-list-item-name"
            ).text(room.get("name"));
            this.$(
                ".lcb-rooms-list-item[data-id=" +
                    room.id +
                    "] .lcb-rooms-list-item-description"
            ).text(room.get("description"));
            this.$(
                ".lcb-rooms-list-item[data-id=" +
                    room.id +
                    "] .lcb-rooms-list-item-participants"
            ).text(room.get("participants"));
        },
        updateLastActive: function (room) {
            this.$(
                ".lcb-rooms-list-item[data-id=" +
                    room.id +
                    "] .lcb-rooms-list-item-last-active .value"
                ...
                swal(
                    {
                        ...
                        text:
                            "You're creating a room with a shared password.\n" +
        ...
        addUser: function (user, room) {
            this.$('.lcb-rooms-list-item[data-id="' + room.id + '"]')
                ...
                .prepend(this.userTemplate(user.toJSON()));
        },
        removeUser: function (user, room) {
            this.$('.lcb-rooms-list-item[data-id="' + room.id + '"]')
                .find('.lcb-rooms-list-user[data-id="' + user.id + '"]')
                .remove();
        },
    });
})(window, $, _);
